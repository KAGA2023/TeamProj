...
public:
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Lmmediate Trigger Start"));
		if (Ctx.Data && Ctx.Data->Animation)
			PlayMontageIfAny(Ctx, Ctx.Data->Animation);

		if (OnFire->IsBound()) OnFire->Execute(Ctx);
	}
};
...
public:
	...
	virtual void ExecuteTrigger(const FSkillContext& Ctx,  FOnFireWithCtx* OnFire) override
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("CastingTome Trigger Start"));
		...
		if (Ctx.Data && Ctx.Data->Animation)
			...
			{																//시전 도중 객체파괴 자동콜백가ㅇ능
				World->GetTimerManager().SetTimer(CastTimeHandle, FTimerDelegate::CreateWeakLambda(this, [this]()
					{
						if (bCasting && CurrentOnFire.IsBound()) { CurrentOnFire.Execute(CurrentCtx); }  bCasting = false;  if (CurrentCtx.Data) StopMontageIfAny(CurrentCtx, CurrentCtx.Data->Animation);
					}),
				CastTime, false);
			}
	};
...
//Trigger_AnimNotify(설정하면 거기서 발사 ) , Trigger_Projectile같은건 노티파이, 발사체 델리게이트에서 OnFireㄱㄱ