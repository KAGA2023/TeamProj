...
UShopComponent::UShopComponent()
{
    ...
    m_CurrentTransaction = FShopTransaction();
    m_ShopLevel = 1;
}
...
{
    UDataTable* ItemDataTable = LoadObject<UDataTable>(nullptr, TEXT("/Game/Data/Item/DT_ItemData.DT_ItemData"));
    if (!ItemDataTable)
        return;
...
bool UShopComponent::PurchaseItem(FName ItemID, int32 Count)
{
    ...
    {
        //거래정보
        FShopPurchaseInfo PurchaseInfo(ItemID, Count, TotalCost);
        m_CurrentTransaction.Purchases.Add(PurchaseInfo);
        m_CurrentTransaction.TotalSpent += TotalCost;
...
bool UShopComponent::SellItem(FName ItemID, int32 Count)
{
    ...
    {
        //거래정보
        FShopSellInfo SellInfo(ItemID, Count, TotalEarned);
        m_CurrentTransaction.Sells.Add(SellInfo);
        m_CurrentTransaction.TotalEarned += TotalEarned;
...
void UShopComponent::CompleteTransaction()
{
    if (m_CurrentTransaction.Purchases.Num() > 0 || m_CurrentTransaction.Sells.Num() > 0)
    {
        OnShopTransaction.Broadcast(m_CurrentTransaction);
    }

    ClearTransaction();
}

void UShopComponent::ClearTransaction()
{
    m_CurrentTransaction = FShopTransaction();
}
...
{
    UShopItem* ShopItem = GetShopItem(ItemID);
    return ShopItem ? ShopItem->GetSellPrice(ItemID) : 0;
}
...
FShopTransaction UShopComponent::GetCurrentTransaction() const
{
    return m_CurrentTransaction;
}

int32 UShopComponent::GetTransactionTotalSpent() const
{
    return m_CurrentTransaction.TotalSpent;
}

int32 UShopComponent::GetTransactionTotalEarned() const
{
    return m_CurrentTransaction.TotalEarned;
}
