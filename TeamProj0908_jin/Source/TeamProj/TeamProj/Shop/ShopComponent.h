...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnShopTransaction, FShopTransaction, Transaction);
...
protected:
    ...
    UPROPERTY(EditAnywhere)
    FShopTransaction m_CurrentTransaction;  //거래 정보
    ...
    UPROPERTY(BlueprintAssignable)
    FOnShopTransaction OnShopTransaction;  //거래 완료 델리게이트
...
public:
    ...
    UFUNCTION(BlueprintCallable)
    void CompleteTransaction();  //거래 완료

    UFUNCTION(BlueprintCallable)
    void ClearTransaction();  //거래 초기화
    ...
    UFUNCTION(BlueprintPure)
    FShopTransaction GetCurrentTransaction() const;  //거래 정보 가져오기

    UFUNCTION(BlueprintPure)
    int32 GetTransactionTotalSpent() const;  //거래 총 지출

    UFUNCTION(BlueprintPure)
    int32 GetTransactionTotalEarned() const;  //거래 총 획득
