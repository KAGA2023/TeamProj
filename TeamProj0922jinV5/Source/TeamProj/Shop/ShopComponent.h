...
protected:
    ...
    // 플레이어의 ItemComponent 참조 (퍼크로 변경된 아이템 데이터 가져오기용)
    UPROPERTY()
    class UItemComponent* m_PlayerItemComponent;
...
public:
    ...
    UFUNCTION(BlueprintCallable)
    void SetupShopComp(int32 SlotCount, int32 RefreshCount);  //상점 설정 (칸 갯수, 리롤권 갯수) <----외부에서 부를 때 얘 부르면 됨
