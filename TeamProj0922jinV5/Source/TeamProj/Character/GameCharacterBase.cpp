...
APlayerCharacter::APlayerCharacter()
{
	...
	ItemComp = CreateDefaultSubobject<UItemComponent>(TEXT("ItemComponent"));  //추가
...
void AEnemyCharacter::BeginPlay()  //추가 자체 수정
{
	Super::BeginPlay();

	// StatusComp 초기화 상태 확인
	if (!StatusComp)
	{
		return;
	}
	
	// StatusComp가 초기화되었는지 확인 (ClassType이 None이 아닌지)
	...
	// 새로 스폰된 적군에게 PlayerState의 PerkComponent에서 적용된 모디파이어들 적용
...
void AEnemyCharacter::ApplyPerksToNewEnemy()  //추가
{
	// 새로 스폰된 적군에게 PlayerState의 PerkComponent에서 적용된 모디파이어들 적용
	APawn* Pawn = UGameplayStatics::GetPlayerPawn(GetWorld(), 0);
	if (Pawn)
	{
		...
		if (GamePlayerState)
		{
			if (UPerkComponent* PerkComp = GamePlayerState->PerkComp)
			{
				...
				// 모든 적군에게 적용되는 모디파이어 적용
				PerkComp->ApplyAllEnemyStatModifiersToNewEnemy(StatusComp);
				
				// 특정 무기 타입 적군에게 적용되는 모디파이어 적용 (새로운 통합 시스템)
...
void AEnemyCharacter::ApplyPerksAfterInit()  //추가
{
	
	// StatusComp가 초기화되었는지 다시 확인
...
void AAllyCharacter::BeginPlay()  //추가 자체 수정
{
	Super::BeginPlay();
...
void AAllyCharacter::ApplyPerksToNewAlly()  //추가
{
	// 새로 스폰된 아군에게 PlayerState의 PerkComponent에서 적용된 모디파이어들 적용
	APawn* Pawn = UGameplayStatics::GetPlayerPawn(GetWorld(), 0);
	if (Pawn)
	{
		...
		if (GamePlayerState)
		{
			if (UPerkComponent* PerkComp = GamePlayerState->PerkComp)
			{
				...
				// 특정 무기 타입 유닛에게 적용되는 모디파이어 적용 (새로운 통합 시스템)
...
void AAllyCharacter::ApplyPerksAfterInit()  //추가
{
