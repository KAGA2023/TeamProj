...
int32 UItemComponent::GetItemSellPrice(FName ItemID) const
{
	...
	int32 SellPrice = OriginalPrice / 2; // ���� ������ 50%
...
void UItemComponent::UpdateItemData(FName ItemID, const FItemData& NewItemData)  //추가
{
	UItem* Item = GetAllItem(ItemID);
	if (Item)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("ItemComponent: Item data updated for %s"), *ItemID.ToString());
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("ItemComponent: Item not found for ID %s"), *ItemID.ToString());
	}
}