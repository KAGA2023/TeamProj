...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnSkillDataChanged, FName, SkillID);    //스킬 데이터 변경 델리게이트
...
protected:
	...
	UPROPERTY(BlueprintAssignable)				FOnSkillDataChanged OnSkillDataChanged;  //추가
...
public:
	...
	bool BeginCast(int32 Index, AActor* OptionalTarget, const FVector& OptionalLocation)
	{
		...
		PendingCtx.Data = S->GetSkillDataPtr(); // �� �ռ� ���Ѵ�� ref/ptr ��ȯ���� �����߾�� ��
	...
	UFUNCTION(BlueprintCallable)
	void UpdateSkillData(FName SkillID, const FSkillData& NewSkillData);  //추가
