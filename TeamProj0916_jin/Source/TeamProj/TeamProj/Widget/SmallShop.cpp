...
void USmallShop::NativeConstruct()
{
	...
	if (RefreshButton)
	{
		RefreshButton->OnClicked.AddDynamic(this, &USmallShop::OnRefreshButtonClicked);
	}
...
void USmallShop::OnRefreshButtonClicked()
{
	...
	if (CachedShopComponent->UseRefreshCount())
	{
		...
		SetupShop(CachedShopComponent, CachedItemComponent);
...
{
	// 호버 시 주황색으로 변경
	SetItemSpriteColor(1, FLinearColor(1.0f, 0.5f, 0.0f, 1.0f)); // 주황색
...
void USmallShop::SetItemSpriteColor(int32 SlotIndex, FLinearColor Color)
{
	if (SlotIndex < 1 || SlotIndex > 5) return;
...
void USmallShop::ResetItemSpriteColor(int32 SlotIndex)
{
	if (SlotIndex < 1 || SlotIndex > 5) return;
...
void USmallShop::UpdateRefreshText()
{
