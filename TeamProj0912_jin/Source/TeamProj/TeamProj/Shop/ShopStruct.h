...
struct FShopPurchaseInfo
{
    ...
    FShopPurchaseInfo() = default;

    FShopPurchaseInfo(const FName& InItemID, int32 InCount, int32 InCost)
        : ItemID(InItemID), PurchaseCount(InCount), TotalCost(InCost)
    {}
};

USTRUCT(BlueprintType)
struct FShopSellInfo
{
    GENERATED_BODY()
    ...
    FShopSellInfo() = default;

    FShopSellInfo(const FName& InItemID, int32 InCount, int32 InEarned)
        : ItemID(InItemID), SellCount(InCount), TotalEarned(InEarned)
    {}
};
...
struct FShopTransaction
{
    ...
    UPROPERTY()
    TArray<FShopPurchaseInfo> Purchases;

    UPROPERTY()
    TArray<FShopSellInfo> Sells;
...
public:
    ...
    UFUNCTION(BlueprintCallable)
    bool CanSellItem(AGameCharacterBase* Seller, const FName& ItemID, int32 Count = 1) const;

    UFUNCTION(BlueprintCallable)
    bool SellItem(AGameCharacterBase* Seller, const FName& ItemID, int32 Count = 1);

    UFUNCTION(BlueprintCallable)
    int32 GetSellPrice(const FName& ItemID) const;
