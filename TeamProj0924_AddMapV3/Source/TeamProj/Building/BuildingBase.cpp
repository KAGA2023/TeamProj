...
void ABuildingBase::SomethingInBound()
{
	...
	UE_LOG(LogTemp, Warning, TEXT("tftf loc : %f, %f, %f,"),GetActorLocation().X, GetActorLocation().Y, GetActorLocation().Z);
	UE_LOG(LogTemp, Warning, TEXT("tftf Ext : %f, %f, %f"), bound.BoxExtent.X, bound.BoxExtent.Y, bound.BoxExtent.Z);
		...
		for (auto& Result : Overlaps)
		{
			if (AActor* Actor = Result.GetActor())
			{
				if (Actor->IsA(AGameCharacterBase::StaticClass()))
				{
					...
					UE_LOG(LogTemp, Warning, TEXT("tftf Found Target Actor: %s"), *Actor->GetName());
				}
			}
		}
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("tftf shooooooooooooooot"));
