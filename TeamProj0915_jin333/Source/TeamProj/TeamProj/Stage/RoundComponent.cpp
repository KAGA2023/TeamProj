	...
	for (FSpawnPortal Pt : roundInfo.PortalInfo)
	{
		...
		if (Pt.PortalID.ToString().Split(TEXT("_"), &L, &R))
		{
			if (R.IsNumeric())
			{
				...
				UE_LOG(LogTemp, Warning, TEXT("try to portal"));
				UE_LOG(LogTemp, Warning, TEXT("Invalid PotalPos index: %d"), id - 1);
				...
				if (!SpawnPt)
				{
					UE_LOG(LogTemp, Warning, TEXT("Failed to Spawn Potal!"));
					return;
				}
				SpawnPt->ptData = Pt.MobInfo;
				UE_LOG(LogTemp, Warning, TEXT("Succesed Spawn Potal"));
			}
		}
		else
			UE_LOG(LogTemp, Warning, TEXT("Cant read Split Failed! PortalID: %s"), *Pt.PortalID.ToString());
